...
def qualitative_sample(
    ...
    with console.capture() as capture:
        for _ in track(list(range(samples)), description="Computing stats...", transient=True):
            ...
            scorer = Bleu(4)
            bleu_scores: Dict[str, Dict[str, float]] = defaultdict(dict)
            for ccc, cccp_a in zip(
                (sample.references if not candidates else sample.candidates),
                (sample.references_tokenized_text if not candidates else sample.candidates_tokenized_text),
            ):
                hypothesis = {0: [" ".join(cccp_a)]}
                for cccb, cccp_b in zip(
                    (sample.references if not candidates else sample.candidates),
                    (sample.references_tokenized_text if not candidates else sample.candidates_tokenized_text),
                ):
                    if tuple(cccp_a) != tuple(cccp_b):
                        bleu_scores[ccc][cccb] = scorer.compute_score({0: [" ".join(cccp_b)]}, hypothesis)[0][-1]
            ...
            for idx, elem in enumerate(sorted(mean_distances, key=mean_distances.get)):  # type: ignore
                fmt_string = f"- {elem} (Dist: {mean_distances[elem]:.2f}) (BLEU@4: {np.amax(list(bleu_scores[elem].values())):.4f})"  # noqa: E501
            ...
            console.print()
            console.print(
                f"Mean Leave One Out BLEU@4 score: {np.mean([np.amax(list(v.values())) for v in bleu_scores.values()]):.4f}"  # noqa: E501
            )
            console.print(
                f"Within-Sample mean BERT Embedding distances [Min, Mean, Max]: [{meds['min']:.2f}, {meds['mean']:.2f}, {meds['max']:.2f}]"  # noqa: E501
            )
