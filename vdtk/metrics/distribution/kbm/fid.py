...
def calculate_fid(act1: torch.Tensor, act2: torch.Tensor) -> float:
    mu1, sigma1 = torch.mean(act1, dim=0), torch.cov(act1.transpose(0, 1))  # type: ignore
    mu2, sigma2 = torch.mean(act2, dim=0), torch.cov(act2.transpose(0, 1))  # type: ignore
    ssdiff = torch.sum((mu1 - mu2) ** 2)
    bmm = sigma1 @ sigma2
    covmean2 = _sqrtm(bmm).real
    return (ssdiff + torch.trace(sigma1 + sigma2 - 2.0 * covmean2)).cpu().item()  # type: ignore
