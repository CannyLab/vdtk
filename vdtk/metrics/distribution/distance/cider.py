...
class CiderScorer(object):
    ...
    def __init__(self, df_mode="corpus", test=None, refs=None, n=4, sigma=6.0):
        ...
        if self.df_mode != "corpus":
            with open(os.path.join(os.path.dirname(__file__), "data", f"{self.df_mode}.pkl"), "rb") as f:
...
class CIDERDDistance(DistanceFunction):
    ...
    def __call__(self, x: str, y: str) -> float:
        return 10 - self.scorer.compute_score([coco_normalize(x)], [coco_normalize(y)])[0]
...
class CIDERDScore(DistanceFunction):
    ...
    @lru_cache(None)
    def __call__(self, x: str, y: str) -> float:
        sc = self.scorer.compute_score([coco_normalize(x)], [coco_normalize(y)])[0]
        # print(coco_normalize(x), '|', coco_normalize(y), sc)
        return sc
